//
//  Images.swift
//  WaveForm
//
//  Created by Eugeni Lazakovich on 24.03.23.
//

import UIKit

public enum Images {
    static let girlImage = UIImage(named: "girl") ?? UIImage()
    static let speakerImage = UIImage(named: "speaker") ?? UIImage()
    static let videoImage = UIImage(named: "video") ?? UIImage()
    static let muteImage = UIImage(named: "mute") ?? UIImage()
    static let endCallImage = UIImage(named: "endCall") ?? UIImage()
    static let statusEndImage = UIImage(named: "call") ?? UIImage()
    static let closeImage = UIImage(named: "closeButton") ?? UIImage()
    static let loaderImage = UIImage(named: "loader") ?? UIImage()
    static let addImage = UIImage(named: "add") ?? UIImage()
    static let starImage = UIImage(named: "star") ?? UIImage()
    static let maskStarImage = UIImage(named: "maskStar") ?? UIImage()
    static let blockImage = UIImage(named: "block") ?? UIImage()
    static let qrImage =  UIImage(named: "qr") ?? UIImage()
    static let editImage = UIImage(named: "edit") ?? UIImage()
    static let successImage = UIImage(named: "success") ?? UIImage()
    static let addUserImage = UIImage(named: "addUser") ?? UIImage()
    static let noContactsImage = UIImage(named: "noContacts") ?? UIImage()
    static let noFavorites = UIImage(named: "noFavorites") ?? UIImage()
    static let noRecents = UIImage(named: "noRecents") ?? UIImage()
    static let godHand = UIImage(named: "godHand") ?? UIImage()
    static let magicWand = UIImage(named: "magicWand") ?? UIImage()
    static let exit = UIImage(named: "exit") ?? UIImage()
}
